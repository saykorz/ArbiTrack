...
using AutoMapper;
using ArbiTrack.Components.Library.Helpers;
...
//https://github.com/jfversluis/MauiWindowsFullscreenMinimizeMaximizeSample
                builder.ConfigureLifecycleEvents(events =>
        {
            ...
            builder.Services.AddDbContext<ApplicationDbContext>(options =>
                options.UseSqlite($"Filename={ApplicationDbContextFactory.GetFullDatabasePath()}",
                    x => x.MigrationsAssembly("ArbiTrack.Data")));
            ...
            }).Wait(); // блокира старта докато не приключи
        ...
        {
            //string localDbPath = Path.Combine(ApplicationDbContextFactory.GetFullDatabasePath());

            //await DownloadDatabaseAsync(localDbPath, !File.Exists(localDbPath));

            using var scope = services.CreateScope();
            var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
            await db.Database.MigrateAsync();

            await ApplicationDbContextFactory.SeedAsync(db);
            //load init data

        }

        private static async Task DownloadDatabaseAsync(string destinationPath, bool isDownload = false)
        {
            //var registerCreate = await AutonomiNet.Register.Create(ApplicationDbContextFactory.DefaultDbFileName, "9999999999999999999");

            //var vault = await AutonomiNet.Vault.Load();
            //AppCache.DatabaseAddress = await AutonomiNet.Register.GetByName(ApplicationDbContextFactory.DefaultDbFileName);
